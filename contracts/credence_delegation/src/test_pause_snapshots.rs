    ...
    e.as_contract(contract_id, || {
        ...
        for (index, signer) in signers.iter().enumerate() {
            let enabled = storage
                ...
                .unwrap_or(false);
            snapshot_signers.insert(format!("signer_{}", index + 1), enabled);
        }
        ...
        for (index, proposal_id) in proposal_ids.iter().enumerate() {
            ...
            for (signer_index, signer) in signers.iter().enumerate() {
                let approved = storage
                    ...
                    .unwrap_or(false);
                approvals.insert(format!("signer_{}", signer_index + 1), approved);
            }
            proposals.insert(
                format!("proposal_{}", index + 1),
                ProposalSnapshot {
...
fn test_pause_proposal_lifecycle_snapshots() {
    ...
    assert_pause_snapshot("06_unpause_proposed", &e, &contract_id, &all_signers, &[prop_id, unpause_id]);

    client.approve_pause_proposal(&signer1, &unpause_id);
    assert_pause_snapshot("07_unpause_approved", &e, &contract_id, &all_signers, &[prop_id, unpause_id]);

    client.execute_pause_proposal(&unpause_id);
    assert_pause_snapshot("08_unpause_executed", &e, &contract_id, &all_signers, &[prop_id, unpause_id]);

    client.set_pause_signer(&admin, &signer2, &false);
    assert_pause_snapshot("09_signer_removed", &e, &contract_id, &all_signers, &[prop_id, unpause_id]);
}
...
fn pause_proposal_remains_pending_when_approvals_are_below_threshold() {
    ...
    let signer1 = Address::generate(&e);
    let signer2 = Address::generate(&e);
    let signers = vec![signer1.clone(), signer2.clone()];
