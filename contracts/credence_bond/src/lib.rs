...
impl CredenceBond {
    ...
    /// Backward-compatible entrypoint for callers that explicitly name the
    /// rolling-bond creation path.
    ...
    fn acquire_lock(e: &Env) {
        ...
        e.storage().instance().set(&Symbol::new(e, "locked"), &true);
    }

    fn release_lock(e: &Env) {
        e.storage()
            .instance()
            .set(&Symbol::new(e, "locked"), &false);
    }
