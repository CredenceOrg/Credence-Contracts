...
fn setter_emits_param_updated_with_old_and_new() {
    ...
    let found = events.iter().any(|(_id, topics, data)| {
        let is_param = topics
            ...
            .map(|t| t == Symbol::new(&e, "param_updated"))
            .unwrap_or(false);
        let is_grace = topics
            .get(1)
            .and_then(|t| Symbol::try_from_val(&e, &t).ok())
            .map(|t| t == Symbol::new(&e, "grace_window"))
            .unwrap_or(false);
        let payload = <(i128, i128)>::try_from_val(&e, &data).ok();
        is_param && is_grace && payload == Some((0, 120))
    });
...
fn setter_event_carries_previous_value() {
    ...
    assert_eq!(<(i128, i128)>::try_from_val(&e, &data).unwrap(), (50, 75));
}
