...
fn test_snapshot_tier_bronze() {
    ...
    client.create_bond_with_rolling(&identity, &500_000_000_i128, &credence_math::Timestamp::SECONDS_PER_DAY, &false, &0_u64);
...
fn test_snapshot_tier_silver() {
    ...
    client.create_bond_with_rolling(&identity, &1_000_000_000_i128, &credence_math::Timestamp::SECONDS_PER_DAY, &false, &0_u64);
    let snap = client.get_bond_status_snapshot();
...
fn test_snapshot_tier_gold() {
    ...
    client.create_bond_with_rolling(&identity, &5_000_000_000_i128, &credence_math::Timestamp::SECONDS_PER_DAY, &false, &0_u64);
    let snap = client.get_bond_status_snapshot();
...
fn test_snapshot_tier_platinum() {
    ...
    client.create_bond_with_rolling(&identity, &20_000_000_000_i128, &credence_math::Timestamp::SECONDS_PER_DAY, &false, &0_u64);
    let snap = client.get_bond_status_snapshot();
...
fn test_snapshot_cooldown_active() {
    ...
    e.ledger().with_mut(|li| li.timestamp = 1000);
    let (client, admin, identity, ..) = setup(&e);
    client.create_bond_with_rolling(&identity, &1_000_000_000_i128, &credence_math::Timestamp::SECONDS_PER_DAY, &true, &3600_u64);
    client.set_cooldown_period(&admin, &3600_u64);
...
fn test_snapshot_available_balance_unslashed() {
    let e = Env::default();
    let (client, _admin, identity, ..) = setup(&e);
    client.create_bond_with_rolling(&identity, &1_000_i128, &credence_math::Timestamp::SECONDS_PER_DAY, &false, &0_u64);
    let snap = client.get_bond_status_snapshot();
...
fn test_snapshot_tier_boundary_bronze_max() {
    ...
    client.create_bond_with_rolling(&identity, &999_999_999_i128, &credence_math::Timestamp::SECONDS_PER_DAY, &false, &0_u64);
